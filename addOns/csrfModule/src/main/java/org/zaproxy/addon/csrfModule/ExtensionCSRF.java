...
public class ExtensionCSRF extends ExtensionAdaptor {
	...
	protected static final String PREFIX = "csrfModule";
	...
	private static final ImageIcon ICON = new ImageIcon(ExtensionCSRF.class.getResource(RESOURCES + "/cake.png"));
	...
	public boolean canUnload() {
		// The extension can be dynamically unloaded, all resources used/added can be
		// freed/removed
		// from core.
		return true;
	}
	...
	public void unload() {
		...
		// In this example it's not necessary to override the method, as there's nothing
		// to unload
		// manually, the components added through the class ExtensionHook (in
		// hook(ExtensionHook))
		// are automatically removed by the base unload() method.
		// If you use/add other components through other methods you might need to
		// free/remove them
	...
	private ZapMenuItem getMenuExample() {
		if (menuExample == null) {
			...
			menuExample.addActionListener(e -> {
				// This is where you do what you want to do.
				// In this case we'll just show a popup message.
				View.getSingleton().showMessageDialog(Constant.messages.getString(PREFIX + ".topmenu.tools.msg"));
		...
		try {
			...
			if (!f.exists()) {
				...
				View.getSingleton().showWarningDialog(
						Constant.messages.getString(ExtensionCSRF.PREFIX + ".error.nofile", f.getAbsolutePath()));
				return;
			}
	...
	private RightClickMsgMenu getPopupMsgMenuExample() {
		if (popupMsgMenuExample == null) {
			popupMsgMenuExample = new RightClickMsgMenu(this, Constant.messages.getString(PREFIX + ".popup.title"));
			this.popupMsgMenuExample.addActionListener(e -> showScanDialog(null));
		}
	...
	public void showScanDialog(SiteNode node) {
		if (dialog == null) {
			dialog = new CSRFDialog(this, View.getSingleton().getMainFrame(),
					DisplayUtils.getScaledDimension(700, 200));
			dialog.init(new Target(node));
		} else if (node != null) {
			dialog.init(new Target(node));
		}
